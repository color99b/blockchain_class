...
class Block extends BlockHeader {
  ...
  updateBlock(difficulyOptions) {
    //난이도와 논스를 사용해서 문제를 푼다.
    // 문제는 difficulty 알고리즘 이라고 한다.
    // difficulty 알고리즘은 2진수로 변화하여 앞의 0의 개수와
    // difficulty와 비교하여 difficulty보다 0의 개수가 많으면
    // 문제를 해결한 것이다.
    // -Block 의 암호화된 hash는 64자의 16진수 수로이루어져있다.
    // - hash를 2진수로 바꾸고 2신수의 수의 제일 앞에서부터 연속되는 0의 개수가
    // difficulty 보다 크면 해결한 것이고 아니면 해결하지 못한 것이다.
    // ex -> hash == AAAA -> 1010 1010 1010 1010 (1부터시작 -> 0이 0개)
    //      difficulty가 0이면 "0"이 0개 있으면 해결이다. 즉 현재는 해결
    //difficulty가 1이면 "0"이 1개 있으면 해결 즉 현재는 해결하지 못했다.
    //  ex 2 -> hash == 1AAA -> 0001 1010 1010 1010
    //          difficulty가 3미만이면 해결못함, 3이상이면 해결.
    let hashBinary = hexToBinary(this.hash);
    // 현재 hash를 2진수로 변환한다.
    while (!hashBinary.startsWith("0".repeat(this.difficulty))) {
      //startsWith 메서드는 string의 메서드로 시작하는 문자를 확인해준다.
      this.nonce += 1;
      //hash가 변경될 수 있도록 nonce를 증가시킨다.
      this.setTimestamp();
      //블록 생성 시간은 chain 에 추가되는 시간이기 때문에 문제 풀이 시점을
      // 생성 시간으로 재설정한다.
      this.getDifficulty(difficulyOptions);
      // 시간이 다시 설정됐기 때문에 기준 시간과 비교하여 난이도를 재설정한다.
      // difficultyOptions라는 변수로 넣은 이유는 updateBlock 메서드 또한
      // 매개변수로 해당 정보를 받아와야하기 때문이다.
      this.hash = Block.createHash(this);
      hashBinary = hexToBinary(this.hash);
      //2진수로 바꾸어 while의 조건문에 해당하지 않는지 확인한다
      // while 조건문이 부정이기 때문에 해당하지 않으면 문제 해결이다.
    }
