...
class Chain implements IChain {
  ...
  isVaildChain(_chain: Array<IBlock>): TResult<undefined, string> {
    //다른 서버에서 체인 받았을때 정상적인 체인인지 확인한다.
    for (let i = 1; i < _chain.length; i++) {
      const nowBlock = _chain[i];
      const previousBlock = _chain[i - 1];
      const isVaild = Block.isValidBlock(nowBlock, previousBlock);
      if (isVaild.isError == true) return isVaild;
    }
    return { isError: false, value: undefined };
  }

  replaceChain(_chain: IBlock[]): TResult<undefined, string> {
  ...
  mineBlock(_address: string) {
    ...
    //코인베이스 트랜젝션의 특징으로 txOutIndex를 블록의 높이로 정의한다.
